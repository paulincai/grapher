 ...
 * @returns {*}
 */
async function fetch(node, parentObject, fetchOptions = {}) {
    ...
    if (parentObject) {
        ...
        results = await accessor.find(filters, options).fetchAsync();
    } else {
        results = await node.collection.find(filters, options).fetchAsync();
    }
