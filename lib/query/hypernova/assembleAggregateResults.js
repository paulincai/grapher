 ...
 */
export default async function(childCollectionNode, aggregateResults, metaFilters) {
    ...
    let allResults = [];
    const results = await childCollectionNode.parent.results
    ...
    if (isMeta && isMany) {
        ...
        const parentResults = await childCollectionNode.parent.results;
