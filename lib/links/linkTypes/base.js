...
export default class Link {
   ...
   * @returns {*|{content}|any}
   */
  async fetch (filters, options, ...others) {
    const result = await this.find(filters, options, ...others).fetchAsync()
   ...
   * @param  {...any} others
   */
  async fetchAsArray (filters, options, ...others) {
    return await this.find(filters, options, ...others).fetchAsync()
  }
  ...
  async _validateIds (ids) {
    ...
    const validIds = await this.linkedCollection.find({
      _id: { $in: ids }
    }, { fields: { _id: 1 } }).fetchAsync().map(doc => doc._id)
  ...
  _virtualAction (action, what, metadata) {
    ...
    // it's an unset operation most likely.
    if (what === undefined) {
    ...
    what = _.map(what, element => {
      if (!_.isObject(element)) {
        ...
        if (!element._id) {
          const elementId = linker.mainCollection.insertAsync(element)
          _.extend(element, linker.mainCollection.findOneAsync(elementId))
        }
